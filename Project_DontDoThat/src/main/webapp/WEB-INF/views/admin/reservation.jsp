...
<c:if test="${listCount > 0}">
	<c:if test="${startPage > pageBlock}">
		<a href="allListReservation.do?num=${userSession.num}&pageNum=${startPage - 1}" title="이전 페이지"><</a>
	</c:if>
	<c:forEach var="i" begin="${startPage}" end="${endPage}">
		<a href="allListReservation.do?num=${userSession.num}&pageNum=${i}"><c:out value="${i}" /></a>
	</c:forEach>
	<c:if test="${endPage < pageCount}">
		<a href="allListReservation.do?num=${userSession.num}&pageNum=${startPage + pageBlock}" title="다음 페이지">></a>
	</c:if>
</c:if>
	</div>
	</c:otherwise>
